...
def main():
    ...
    try:
        ...
        with st.expander("🌎 Spanish-Speaking Markets Analysis", expanded=False):
            ...
            # --------------------------------------------------
        # Country → Top Pages Performance
        # --------------------------------------------------
        st.markdown(
            '<h2 class="section-header">Country → Top Pages Performance</h2>',
            unsafe_allow_html=True
        )
        ...
        country_pages_df = filtered_df[
            filtered_df['country'] == selected_country
        ]

        if len(country_pages_df) == 0:
            st.info("No data available for this country.")
        else:
            # Aggregate page-level performance
            top_pages = (
                country_pages_df
                .groupby('page')
                .agg(
